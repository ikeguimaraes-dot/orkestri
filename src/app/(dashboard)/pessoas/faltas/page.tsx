    ...
    <div style={{ maxWidth: 1180, margin: "0 auto" }}>
      <header style={{ marginBottom: 20 }}>
        ...
        <p style={{ fontSize: 12, color: "var(--text-3)", margin: 0, lineHeight: 1.55, maxWidth: 720 }}>
          Registro consolidado por unit. Faltas injustificadas descontam score automaticamente.
        </p>
      </header>
      ...
      <Suspense fallback={<div style={{ color: "var(--text-3)", fontSize: 13, marginTop: 12 }}>Carregando…</div>}>
        <FaltasSection mes={mes} ano={ano} />
      </Suspense>
    </div>
  );
}

async function FaltasSection({ mes, ano }: { mes: number; ano: number }) {
